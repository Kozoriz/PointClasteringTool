...
QString QtRedebleOject::cloudChoosen()
{
  return m_choosenCloud;
}

QVariant QtRedebleOject::newPoint()
{
  return m_newPoint;
}

QString QtRedebleOject::addedCloudName()
{
  return m_addedCloudName;
}
...
void QtRedebleOject::setCloudChoosen(QString cloudname)
{
  LOG_AUTO_TRACE();
  m_choosenCloud = cloudname;
  m_renderer->cloudChoosen(m_choosenCloud.toLocal8Bit().constData());
}

void QtRedebleOject::addNewPoint(QVariant point)
{
//  LOG_AUTO_TRACE(); // too high count of calls will be
  m_newPoint = point;

  emit newPointAdded();
}

void QtRedebleOject::addCloudToList(QString name)
{
  m_addedCloudName = name;
